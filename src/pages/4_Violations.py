...
VIOLATION_TYPE_FILTER_OPTIONS = [
    "All Types", "Speeding", "Reckless Driving", "Illegal Parking", "Obstruction",
...
def filter_dialog():
    ...
    with col1:
        stat_idx = STATUS_FILTER_OPTIONS.index(cur["violation_status"]) \
                   if cur["violation_status"] in STATUS_FILTER_OPTIONS else 0
        violation_status = st.selectbox("Payment Status", STATUS_FILTER_OPTIONS, index=stat_idx)
    with col2:
        vtype_idx = VIOLATION_TYPE_FILTER_OPTIONS.index(cur["violation_type"]) \
                    if cur["violation_type"] in VIOLATION_TYPE_FILTER_OPTIONS else 0
        violation_type = st.selectbox("Violation Type", VIOLATION_TYPE_FILTER_OPTIONS, index=vtype_idx)
    ...
    if date_mode == "Single Year":
        ...
        yr_idx = year_options.index(new_date_year) if new_date_year in year_options else 0
...
def edit_violation_dialog(v):
    ...
    with st.form("edit_violation_form"):
        ...
        with col2:
            ...
            new_fine    = st.number_input("Fine Amount (₱)", min_value=0, step=100,
...
if not violations:
    ...
    if df.empty:
        ...
        display_cols = [
            "TOP Number", "Violation Type", "Date & Time", "Location",
            "License No.", "Plate No.", "Fine (₱)", "Status", "Officer",
        ]
        ...
        if not r7_location.strip():
            ...
            try:
                ...
                if not results:
                    ...
                    r7_df = pd.DataFrame(results).rename(columns={
                        "top_number":           "TOP Number",
                        "violation_type":       "Violation Type",
                        "violation_date":       "Date & Time",
                        "location":             "Location",
                    ...
                    r7_display_cols = [
                        "TOP Number", "Violation Type", "Date & Time",
                        "Location", "License No.", "Plate No.", "Fine (₱)", "Status",
                    ]
