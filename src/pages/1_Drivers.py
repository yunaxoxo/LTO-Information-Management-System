...
st.markdown(
    "Manage and monitor driver information, including licenses, violations, and other details."
)
st.markdown("---")
...
def filter_dialog():
    ...
    type_idx = (
        LICENSE_TYPE_OPTIONS.index(cur["license_type"])
        if cur["license_type"] in LICENSE_TYPE_OPTIONS
        else 0
    )
    status_idx = (
        LICENSE_STATUS_OPTIONS.index(cur["license_status"])
        if cur["license_status"] in LICENSE_STATUS_OPTIONS
        else 0
    )
    ...
    license_status = st.selectbox(
        "License Status", LICENSE_STATUS_OPTIONS, index=status_idx
    )
    ...
    with age_col1:
        min_age = st.number_input(
            "Min Age", min_value=0, max_value=150, value=cur["min_age"], step=1
        )
    with age_col2:
        max_age = st.number_input(
            "Max Age", min_value=0, max_value=150, value=cur["max_age"], step=1
        )
    ...
    with st.form("add_driver_form"):
        ...
        with col1:
            license_number = st.text_input(
                "License Number *", placeholder="A01-15-100001"
            )
            full_name = st.text_input("Full Name *", placeholder="Juan Dela Cruz")
            birthday = st.date_input(
                "Birthday *", min_value=date(1900, 1, 1), max_value=date.today()
            )
        ...
        with col2:
            ...
            expiration_date = st.date_input(
                "Expiration Date *",
                value=issuance_date + timedelta(days=1),
                min_value=issuance_date + timedelta(days=1),
            )
        if st.form_submit_button(
            "Add Driver", use_container_width=True, type="primary"
        ):
...
def delete_driver_dialog(d):
    st.warning(
        f"This will permanently delete **{d['Full Name']}** and all linked records."
    )
    ...
    with st.form("edit_driver_form"):
        ...
        with c1:
            ...
            new_bday = st.date_input(
                "Date of Birth", value=d.get("Birthday") or date.today()
            )
            new_sex = st.selectbox(
                "Sex", ["M", "F"], index=0 if d.get("Sex") == "M" else 1
            )
        ...
        with c2:
            ...
            issue_val = d.get("Issuance Date")
            if isinstance(issue_val, str):
                from datetime import datetime

                issue_val = datetime.strptime(issue_val, "%Y-%m-%d").date()
            elif not isinstance(issue_val, date):
                issue_val = date.today()

            new_issue = st.date_input("Issuance Date", value=issue_val)

            exp_val = d.get("Expiration Date")
            if isinstance(exp_val, str):
                from datetime import datetime

                exp_val = datetime.strptime(exp_val, "%Y-%m-%d").date()
            elif not isinstance(exp_val, date):
                exp_val = date.today()

            min_exp_val = new_issue + timedelta(days=1)
            if exp_val < min_exp_val:
                exp_val = min_exp_val

            new_exp = st.date_input(
                "Expiration Date", value=exp_val, min_value=min_exp_val
            )
...
if not drivers:
    ...
    if search_query:
        ...
        mask = df["Full Name"].str.lower().str.contains(sq, na=False) | df[
            "License Number"
        ].str.lower().str.contains(sq, na=False)
        df = df[mask]
    ...
    if df.empty:
        ...
        paginated_df, start_idx, total_pages = paginate_df(
            df, "current_page", rows_per_page=10
        )
        ...
        if selected_rows:
            ...
            with action_bar.container():
                with st.container(border=True):
                    ...
                    with c_edit:
                        if st.button(
                            "Edit", use_container_width=True, key="edit_driver"
                        ):
