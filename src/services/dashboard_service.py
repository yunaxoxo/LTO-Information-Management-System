...
def get_valid_licenses_count() -> int:
    res = execute_query(
        "SELECT COUNT(*) AS val FROM drivers WHERE license_status = 'Valid'", ()
    )
    return res[0]["val"] if res else 0
...
def get_expired_licenses_count() -> int:
    res = execute_query(
        "SELECT COUNT(*) AS val FROM drivers WHERE license_status = 'Expired'", ()
    )
    return res[0]["val"] if res else 0
...
def get_suspended_licenses_count() -> int:
    res = execute_query(
        "SELECT COUNT(*) AS val FROM drivers WHERE license_status IN ('Suspended','Revoked')",
        (),
    )
    return res[0]["val"] if res else 0
...
def get_expired_registrations_count() -> int:
    res = execute_query(
        "SELECT COUNT(*) AS val FROM vehicle_registrations WHERE registration_status = 'Expired'",
        (),
    )
    return res[0]["val"] if res else 0
...
def get_contested_violations_count() -> int:
    res = execute_query(
        "SELECT COUNT(*) AS val FROM traffic_violations WHERE violation_status = 'Contested'",
        (),
    )
    return res[0]["val"] if res else 0
...
def get_total_fines_collected() -> float:
    res = execute_query(
        "SELECT COALESCE(SUM(fine_amount),0) AS val FROM traffic_violations WHERE violation_status='Paid'",
        (),
    )
